...
func (c *Consumer) worker(ctx context.Context, msgs <-chan amqp091.Delivery, workerID int) {
	defer c.wg.Done()
...
func (c *Consumer) processMessage(ctx context.Context, msg amqp091.Delivery, workerID int) {
	...
	// Validate payload
	if payload.UserID == 0 {
		c.log.WithField("worker_id", workerID).Error("invalid user_id in message")
		_ = msg.Nack(false, false)
		return
	}
